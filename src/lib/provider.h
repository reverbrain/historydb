...
namespace history {
	...
	public:
		...
		virtual std::map<std::string, uint32_t> get_active_user(uint64_t time);
		virtual std::map<std::string, uint32_t> get_active_user(const std::string& key);
		...
		virtual void for_active_user(uint64_t time, std::function<bool(const std::string& user, uint32_t number)> func);
		virtual void for_active_user(const std::string& key, std::function<bool(const std::string& user, uint32_t number)> func);
	...
	private:
		...
		uint32_t get_key_spread_size(ioremap::elliptics::session& s, const std::string& key);
