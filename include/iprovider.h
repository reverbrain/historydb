...
namespace history
{
	...
	public:
		...
		*/
		virtual void add_user_activity(const std::string& user, uint64_t time, void* data, uint32_t size, const std::string& key = std::string()) const = 0;
		...
		*/
		virtual void repartition_activity(const std::string& key, uint32_t parts) const = 0;
		virtual void repartition_activity(const std::string& old_key, const std::string& new_key, uint32_t parts) const = 0;
		virtual void repartition_activity(uint64_t time, uint32_t parts) const = 0;
		virtual void repartition_activity(uint64_t time, const std::string& new_key, uint32_t parts) const = 0;

		/* Gets user's logs for specified period
		*/
		virtual std::list<std::vector<char>> get_user_logs(const std::string& user, uint64_t begin_time, uint64_t end_time) const = 0;
		/* Gets active users with activity statistics for specified day
		*/
		virtual std::map<std::string, uint32_t> get_active_user(uint64_t time) const = 0;
		/* Gets active users with activity statistics for specified key
		*/
		virtual std::map<std::string, uint32_t> get_active_user(const std::string& key) const = 0;


		/* Iterates by user's logs and calls func for each log record
		*/
		virtual void for_user_logs(const std::string& user, uint64_t begin_time, uint64_t end_time, std::function<bool(const std::string& user, uint64_t time, void* data, uint32_t size)> func) const = 0;

		/* Iterates by activity statistics and calls func for each user
		*/
		virtual void for_active_user(uint64_t time, std::function<bool(const std::string& user, uint32_t number)> func) const = 0;
